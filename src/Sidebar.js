    ...
    return (
        <div className="md:bg-gh-sidebar px-6 py-6 flex flex-col space-y-5 border-r border-gray-800 border-opacity-90">
            ...
            <div className="text-sm">
                <Input value={repoSearch} onChange={e => setRepoSearch(e.target.value)} placeholder="Find a repository..." className="bg-gh border border-gray-700 border-opacity-50 w-full"/>
            </div>
            <div className="flex flex-col">
                {repos.filter(repo => repo.name.toLowerCase().includes(repoSearch.toLowerCase()) || repo.username.toLowerCase().includes(repoSearch.toLowerCase())).map(repo => {
                    return (
                ...
                {repos.length === 0 && <div className="px-2 text-sm">
			...
			<div className="text-sm ">
            	<Input value={teamSearch} onChange={e => setTeamSearch(e.target.value)} placeholder="Find a team..." className="bg-gh border border-gray-700 border-opacity-50 w-full"/>
			</div>
            <div className="flex flex-col">
                {teams.filter(team => team.name.toLowerCase().includes(teamSearch.toLowerCase()) || team.username.toLowerCase().includes(teamSearch.toLowerCase())).map(team => {
                    return (
                        <div className="flex items-center space-x-1 cursor-pointer">
                ...
                {teams.length === 0 && <div className="text-sm px-2">
